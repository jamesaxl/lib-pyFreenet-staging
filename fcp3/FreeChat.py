...
# uniqueid is unique between teo users
DB_SCHEMA = '''
...
class FreeChat(object):
    def __init__(self, host = 'localhost', port = 9481, gui = None):
        ...
        if not config_file.exists():
            ...
            config['DEFAULT'] = {  'HOST' : host, 
                                   'PORT' : port,
                                   'NAME' : 'freechat_{0}'.format(str(uuid.uuid4())), }
        ...
        self.node = FCPNode(host = self.get_config()['HOST'], 
    ...
    def generate_key_for_friend(self, owner, friend):
        pub, prv  = self.node.gen_key(usk = True, name = owner + '_to_' + friend)
    ...
    def get_msg(self, friend, owner):
        ...
        with sqlite3.connect(str(db_filename)) as conn:
            ...
            job = self.node.get( uri = self.message_uri_to_get, async = True,
                             priority = 1, callback = self.get_msg_callback, 
