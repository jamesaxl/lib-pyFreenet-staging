...
class SiteMgr(object):
    ...
    def create_site(self, **kw):
        ...
        self.pub, self.prv = self.node.gen_key()

        self.version = 0

        job = self.node.put_directory(uri = self.prv, async = True,
                              usk = True,
                              ...
                              upload_from = 'disk', priority = int(self.get_config()['PRIORITY']), version = self.version,
    ...
    def update_site(self, site_name):
        ...
        with sqlite3.connect(str(db_filename)) as conn:
            ...
            job = self.node.put_directory(uri = prv, async = True,
                                  usk = True,
